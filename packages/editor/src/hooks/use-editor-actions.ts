...
function toPersistedLayout(layout: SceneNode["layout"]): PersistedLayout {
...
function toPersistedNode(node: SceneNode): PersistedNode {
  return {
    id: node.id,
    type: node.type,
    name: node.name,
...
function toPersistedScene(scene: SceneGraph): PersistedScene {
  return {
      ...
      if (result.ok) {
        ...
        setDoc((currentDoc) => {
          ...
          return {
            ...
            scenes: {
              ...currentDoc.scenes,
              [currentPage.sceneId]: toPersistedScene(result.scene),
            },
          };
        });
      }
      ...
      if ("scene" in dispatchResult) {
        ...
        setDoc((d) => {
          ...
          return {
            ...
            scenes: {
              ...d.scenes,
              [page.sceneId]: toPersistedScene(dispatchResult.scene),
            },
          };
        });
      } else {
