...
import { createBatchHandler, computeBatchInverse, batchInverse } from "../src/runtime/handlers/batch.js";
  ...
  return (action: RuntimeAction): DispatchResult => {
    try {
      if (action.type === "create-node") {
        const newScene = createNodeHandler(currentScene, action as any, { now: Date.now });
        currentScene = newScene;
        return { ok: true, scene: newScene };
      }
      if (action.type === "remove-node") {
        const newScene = removeNodeHandler(currentScene, action as any, { now: Date.now });
        currentScene = newScene;
        return { ok: true, scene: newScene };
      }
...
describe("batchInverse", () => {
  it("returns undefined (requires dispatch context)", () => {
    ...
    const inverse = batchInverse(emptyScene, batchAction as any, { now: Date.now });
    expect(inverse).toBeUndefined();
  });
});
