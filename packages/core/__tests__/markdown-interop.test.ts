...
describe("markdownToDoc", () => {
  ...
  it("converts headings", () => {
    ...
    expect((doc.content[0] as { attrs: { level: number } }).attrs.level).toBe(1);
    ...
    expect((doc.content[2] as { attrs: { level: number } }).attrs.level).toBe(3);
  });

  it("converts bold and italic", () => {
    const doc = markdownToDoc("This is **bold** and *italic*");
    expect(validateRichText(doc)).toBe(true);
    const content = (doc.content[0] as { content?: unknown[] }).content!;
  ...
  it("converts links", () => {
    ...
    expect(validateRichText(doc)).toBe(true);
    const content = (doc.content[0] as { content?: Array<{ marks?: Array<{ type: string; attrs?: { href: string } }> }> }).content![0];
  ...
  it("converts bullet lists", () => {
    ...
    expect((doc.content[0] as { content?: unknown[] }).content).toHaveLength(2);
  });
